  ...
  const handleSubmit = () => {
    axios.post("http://localhost:8080/eventos", form)
      .then(() => {
        ...
        setForm({ nmEvento: "", dsEvento: "", dtInicioEvento: "", dtFimEvento: "", local: { cdLocal: "" }, categoria: { cdCategoria: "" }, imagens: [] });
      })
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <input className="border p-2 rounded" placeholder="Nome" value={form.nmEvento} onChange={e => setForm({ ...form, nmEvento: e.target.value })}/>
        <input className="border p-2 rounded" placeholder="Descrição" value={form.dsEvento} onChange={e => setForm({ ...form, dsEvento: e.target.value })}/>
        <input className="border p-2 rounded" type="date" value={form.dtInicioEvento} onChange={e => setForm({ ...form, dtInicioEvento: e.target.value })}/>
        <input className="border p-2 rounded" type="date" value={form.dtFimEvento} onChange={e => setForm({ ...form, dtFimEvento: e.target.value })}/>
        <select className="border p-2 rounded" value={form.local.cdLocal} onChange={e => setForm({ ...form, local: { cdLocal: e.target.value } })}>
